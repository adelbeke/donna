...
export function PRCardActions({ isHidden, isPriority, prUrl, toggleHide, togglePriority}: Props) {
return (
    <div className="flex items-center gap-1 shrink-0 flex-col lg:flex-row">
        <PRCardAction onClick={toggleHide} title={isHidden ? 'Unhide PR' : 'Hide PR'} className={isHidden
            ? 'text-[var(--color-warning)]'
            : 'text-[var(--color-text-muted)] hover:text-[var(--color-warning)]'} >
            {isHidden ? <Eye size={14} /> : <EyeOff size={14} />}
        </PRCardAction>
        <PRCardAction onClick={togglePriority} title={isPriority ? 'Remove priority' : 'Mark as top priority'} className={isPriority
