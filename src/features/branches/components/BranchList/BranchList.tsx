...
export function BranchList() {
  ...
  for (let i = 0; i < localPaths.length; i++) {
    ...
    const repoItems = (branches ?? [])
      .filter((b) => !filters.search || b.toLowerCase().includes(filters.search.toLowerCase()))
      .map((branch) => ({
    ...
    // ponytail: float the current branch to the top of its repo; stable order otherwise.
