...
export function BranchDashboard() {
    ...
    return (
        <div className={"flex flex-col gap-8"}>
            <SearchInput value={filters.search} placeholder={'Filter by branch name…'} displayClearButton={filters.search.length > 0} onChange={handleChange} onClear={handleClear} />
            <BranchList />
        </div>
    )
}